...
# 二、自己编译运行
###### 1.下载源码使用VS2017及以上版本（最好是2017）打开，修改YYS_AssistDlg.cpp中的大漠插件注册（有注释），在g_dm->Reg(_T(""),_T(""));填入自己买的或找的大漠注册码和附加码。
###### 2.使用大漠图色工具（百度一大堆分享的）自己截取游戏不同场景的图片（挺多的，不想截图就用现成的exe支持一下作者(●ˇ∀ˇ●)）放在项目的pic_font文件夹（桌面端也适用，不过得自己在代码中调各个场景识别范围），
